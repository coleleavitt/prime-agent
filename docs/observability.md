...
| `dream.candidate`     | `dream.iteration`, `dream.candidate_index`, `dream.policy_id`, `dream.origin` (`llm`/`local`), `dream.reason` (`winner`/`tie`/`worse`/`quality-rejected`/`unmeasurable`/`identical`/`duplicate`), `dream.eligible`, `dream.value`, `dream.quality`, `dream.anytime`, `dream.cost`, `dream.rounds_saved`, `dream.in_support_min`, `dream.changed` (the policy fields that differ from current, comma-joined) | done | coding-agent `core/dream/improve.ts` (`runDreaming`; one per candidate of a dreaming step, emitted post hoc from the `CandidateVerdict` list as a zero-duration child of `dream.dream` after the selection, so the verdict of every proposed policy is in the trace as well as in the dreams log) |
