...
| `rlm.run_agent`       | `rlm.requested_model`, `rlm.model`, `rlm.requested_thinking`, `rlm.thinking`, `rlm.max_output_tokens`, `rlm.status`, `rlm.turns` | done | coding-agent `core/agent-session.ts` (`ctx.runAgent` children) |
...
| `dream.experiment_arm` | `dream.experiment_id`, `dream.arm`, `dream.fixed_policy`, `dream.guided`, `dream.run_id` | done | coding-agent `core/dream/experiment.ts` / `core/dream/experiment-llm.ts` (one per arm, child of `dream.experiment`; `dream.run_id` is set once the arm's loop has an id and is distinct per arm, since the loop folds `<experimentId>/<arm>` into it) |
| `dream.run`           | `dream.task`, `dream.seed`, `dream.workers`, `dream.k1`, `dream.k2`, `dream.dreams`, `dream.iterations`, `dream.mode` (`local`/`llm`), `dream.fixed_policy`, `dream.priming_policies` (priming rollouts charged to round 1), `dream.run_id` (`<task>-s<seed>-r<clock>[-<runLabel>]`, the key of the run's `dreams/` and `rejections/` logs; an experiment arm's label is `<experimentId>_<arm>`), `dream.stopped` (`aborted`, on cancel), `trigger.trace_id` (LLM path only); LLM path, when the child scope carries them: `dream.child_model`, `dream.child_thinking`, `dream.child_max_output_tokens` | done | coding-agent `core/dream/loop.ts` (`runDreamLoop`, sync local CLI: an in-turn root, or a child of `dream.experiment_arm` under an experiment) and `core/dream/llm.ts` (`runDreamLoopWithAgent`, in-session: a detached root carrying the launching turn's `trigger.trace_id`, ended on success/abort/error; every other `dream.*` span of the loop is an ordinary child of it on both paths). `dream.fixed_policy` marks the control arm, which never dreams. `DreamRunService` opens no span of its own |
...
| `dream.dream`         | `dream.candidates`, `dream.pool_size`, `dream.iteration`, `dream.chosen_policy_id`, `dream.chosen_score`, `dream.current_score`, `dream.chosen_quality`, `dream.current_quality`, `dream.quality_rejected`, `dream.unmeasurable` (candidates whose replay says nothing: replay-dead-only changes or out-of-support replays), `dream.improved`, `dream.dreamer` (`llm`/`local`/`mixed`), `dream.in_support_current` (the current policy's mean in-support share over the whole pool), `dream.measured_trees` (trees the current policy replays in full support: every score is a mean over these only), `dream.evidence_trees` (`measured_trees - 1`, floor 0: the trees whose replays can vouch for a stop-early credit; with 0 every policy is charged the whole budget), `dream.simulations` (replays the selection made), `dream.lever_gap`, `dream.lever_policies`, `dream.lever_simulations` (the fixed lever-scan grid: how far its best eligible policy beats current, 0 when none does, how many grid policies were scored and how many replays that cost; absent when the scan is off) | done | coding-agent `core/dream/improve.ts` (`runDreaming`; one per dreaming step, a child of `dream.run` on both paths). Scores are the replay objective `(1 - beta3) q + beta3 anytime - beta1 S_eff/(W k1) + beta2 (1 - rounds_eff/k1)` with a candidate's spend charged at the latest probe and round it was still improving on the other measured trees and the incumbent's at its raw spend (`docs/dream-rsi.md`, evidence-backed spend); `*_quality` is the mean best normalized to the measured pool, the guard is per tree, and `quality_rejected` counts exactly the candidates whose verdict is `quality-rejected` (an off-support quality failure is `unmeasurable` only, so the eight verdict reasons partition `candidates` and `quality_rejected + unmeasurable` never exceeds it). Every step is also written to `<dreamDir>/dreams/<runId>.jsonl` with one verdict per candidate |
| `dream.replay`        | per dreaming step: `dream.policy_id`, `dream.iteration`, `dream.simulations` (the exact `simulatePolicy` calls the selection made: the current policy on every tree, then each simulated candidate on the measured trees; identical, duplicate and replay-dead-only candidates cost none, and the lever scan's replays are counted on `dream.dream`, not here), `dream.measured_trees`; standalone `dream replay`: `dream.policy_id`, `dream.tree_id`, `dream.revealed_n`, `dream.rounds`, `dream.v`, `dream.out_of_support` (cells charged as probes that revealed nothing), `dream.in_support` (`N / (N + out_of_support)`), `dream.probes_to_best` (1-based charged selection that first reached the best; 0 for the root), `dream.simulations` (1) | done | coding-agent `core/dream/improve.ts` (one coarse replay summary per dreaming step, a child of `dream.dream`, not one per simulation; the step's replays number in the tens, the lever scan's in the hundreds) and `core/dream/replay.ts` (`simulatePolicyWithSpan`, the standalone subcommand's root; bare `simulatePolicy` opens no span) |
| `dream.candidate`     | `dream.iteration`, `dream.candidate_index`, `dream.policy_id`, `dream.origin` (`llm`/`local`), `dream.reason` (`winner`/`tie`/`worse`/`quality-rejected`/`unmeasurable`/`revoked`/`identical`/`duplicate`; `revoked` is a policy the run adopted and reverted after its probation rollout), `dream.eligible`, `dream.value`, `dream.quality`, `dream.anytime`, `dream.cost`, `dream.rounds_saved`, `dream.in_support_min`, `dream.charged_probes`, `dream.charged_rounds` (the mean spend the cost terms charged per tree: `max(N + oos, H_probes)` and `max(rounds, H_rounds)` against the candidate's own probes/rounds to best on the other measured trees, the whole budget when there is none; the incumbent is charged its raw spend), `dream.changed` (the policy fields that differ from current, comma-joined) | done | coding-agent `core/dream/improve.ts` (`runDreaming`; one per candidate of a dreaming step, emitted post hoc from the `CandidateVerdict` list as a zero-duration child of `dream.dream` after the selection, so the verdict of every proposed policy is in the trace as well as in the dreams log) |
| `dream.redeploy`      | `dream.policy_id`, `dream.k1`, `dream.workers`, `dream.iteration`, `dream.tree_id`, `dream.fixed_policy`, `dream.probation` (true when the step adopted a new policy, so this rollout is its probation), `dream.probation_floor`, `dream.reverted` (probation only: the incumbent's lowest replay best over the measured pool, and whether the rollout's best fell below it and the adoption was reverted) | done | coding-agent `core/dream/loop.ts` / `core/dream/llm.ts` (wraps the redeploy rollout of a chosen policy; on a fixed-policy iteration the initial policy is redeployed without dreaming and `dream.fixed_policy` is true; a reverted probation restores the incumbent for the next step, revokes the policy id for the run and writes a `probation` line to `<dreamDir>/dreams/<runId>.jsonl`) |
...
| `dream.llm_dream`     | `dream.candidates_requested`, `dream.iteration`, `dream.tokens`, `dream.llm_attempts` (child calls, the retry included), `dream.candidates_returned` (entries in the child's array), `dream.candidates_dropped` (entries the strict policy parser refused), `dream.candidates_kept` (distinct new policies the child contributed, at most the requested count), `dream.candidates_truncated`, `dream.candidates_local` (the local top-up), `dream.candidates` (total passed to the selection), `dream.dreamer` (`llm`/`mixed`/`local`), `dream.llm_status`, `dream.llm_fallback` (true when the child contributed nothing), `dream.llm_reject_reason` / `dream.llm_reject_excerpt` (the last rejected result's reason and a 240-character head/tail of its output) | done | coding-agent `core/dream/llm.ts` (`proposePoliciesWithAgent`; only on the `--llm-dreamer` path, one per dreaming step, a child of `dream.run`; the JSON array is extracted leniently from the child's output, each entry goes through the strict policy parser, identical and duplicate entries ride along to be audited by the selection, a retryable rejection gets one more call, every rejection is a `role: dreamer` line in `<dreamDir>/rejections/<runId>.jsonl`, and a shortfall is topped up from the local mutator) |
