...
import { runDreamCommand } from "./dream-command.js";
...
async function runPublicCommand(args: string[]): Promise<PublicCommandResult> {
	...
	switch (command) {
		...
		case "dream": {
			const code = runDreamCommand(args.slice(1), { stdout: console.log, stderr: console.error });
			if (code !== 0) {
...
function runTrace(args: string[]): PublicCommandResult {
	const exitCode = runTraceCommand(args, { stdout: console.log, stderr: console.error });
	if (exitCode !== 0) {
		process.exitCode = exitCode;
	}
	return HANDLED;
