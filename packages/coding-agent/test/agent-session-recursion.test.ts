...
describe("AgentSession rlm recursion", () => {
	...
	it("postpones a completion notice while the child keeps working and rewrites its preview on delivery", async () => {
		...
		await vi.waitFor(
			() => {
				...
				expect(notices).toHaveLength(1);
				expect(notices[0]).toMatchObject({
					content: expect.stringContaining("[child-exited: no-reply child:still-working-worker]"),
					details: {
						kind: "completed_without_reply",
						childId: spawned.rlm_child_id,
						lastAssistantTextPreview: "final report text",
					},
				});
			},
			{ timeout: 5000 },
		...
		await vi.waitFor(() => {
			...
			expect(notices[0]).toMatchObject({
				content: expect.stringMatching(
					/^\[child-exited: no-reply child:scheduled-worker\]\n\nThe child ended its turn without sending a reply and still has 1 scheduled work item pending \(soonest next run in \d+m( \d+s)?\): scheduler: 2 tasks scheduled\. The child is waiting, not finished; do not redo its work\./,
				),
				details: {
					kind: "completed_without_reply",
					// The child id left the rendered text with the sanitized header; it is
					// still asserted, here where it now lives.
					childId: spawned.rlm_child_id,
					scheduledWork: [
	...
	it("restores the plain completion wording once the child clears its scheduled work", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(notices).toHaveLength(1);
			expect(notices[0]).toMatchObject({
				content: "[child-exited: no-reply child:cleared-worker]\n\nLast assistant text: last scheduled run done",
	...
	it("regression: fails a child whose terminal response is content [] / stop / usage 0 instead of settling done", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(failures[0]).toMatchObject({
				content: expect.stringContaining("[child-failed child:empty-worker]"),
				details: {
					childId: spawned.rlm_child_id,
					error: expect.stringContaining("empty assistant response (stopReason=stop usage=0"),
				},
			});
		});
		expect(
...
describe("RLM child terminal notice content", () => {
	...
	it("states the pending scheduled work, its count, and the soonest relative next run", () => {
		...
		expect(notice.content).toBe(
			"[child-exited: no-reply child:wakeup-worker]\n\nThe child ended its turn without sending a reply and still has 2 scheduled work items pending (soonest next run in 5m): scheduler: 2 tasks scheduled, watcher. The child is waiting, not finished; do not redo its work.\n\nLast assistant text: build started",
		);
	});
	...
	it("keeps the original wording when no scheduled work is declared", () => {
		...
		).toBe("[child-exited: no-reply child:quiet-worker]\n\nLast assistant text: all done");
		expect(
			createRlmChildTerminalNoticeMessage(
