...
describe("harness digest relevance ranking", () => {
	...
	it("selects top-k per kind by relevance instead of alphabetical order", () => {
		...
		// Without query terms the fallback is rankHarnessEntriesForPrompt, not the
		// old alphabetical order: sorting by [path,title,id] rendered 6 of 27 real
		// entries and pushed a behavioural preference from rank 2 to rank 8, so a
		// correct rule was never shown. Behavioural entries lead, then recency --
		// here both are plain memories, so the newer "zzz" wins the single slot.
		const unranked = formatHarnessStateForPrompt(state, { maxEntriesPerKind: 1 });
		expect(unranked).toContain("[global:zzz]");
		expect(unranked).not.toContain("[global:aaa]");
		expect(unranked).not.toContain("(entries ranked by relevance");
	});
