---
name: dream
description: Start the Dream-RSI loop (rollout, dream a no-worse exploration policy, redeploy) over a scored task from the Python REPL; returns immediately, runs in the background, progress in the Agents View and via status(). Default is local and token-free; llm_proposer/llm_dreamer spend tokens.
---
...
- `await dream.status()` — current run status as a dict (`runId`, `phase`,
  ...
  `stopReason`, ...) or `{"phase": "idle"}` when nothing is running.
...
- Only the `llm_proposer` / `llm_dreamer` paths spend tokens. The default is
  local and token-free.
