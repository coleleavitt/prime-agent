...
async def run(
    ...
    implement_candidates: int | None = None,
    ...
    `implement_candidates` (1..8) makes each implement round produce that many
    candidates in parallel; each is fast-screened and the lexicographic winner
    (screen pass, higher screen score, fewer tokens) goes on to the deep gate.
    It multiplies token cost, so reserve it for expensive targets such as ARC.
    ...
    _check_count("implement_candidates", implement_candidates)
    if implement_candidates is not None and implement_candidates > 8:
        raise ValueError("implement_candidates must be between 1 and 8")
    ...
    if implement_candidates is not None:
        payload["implement_candidates"] = implement_candidates
    if arc_agi is not None:
